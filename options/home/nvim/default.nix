{
  ...
  config = lib.mkIf config.mystuff.nvim-config.enable {
    home-manager.users.${config.mystuff.username} = {
      home.packages = with pkgs; [nil stylua beautysh clang-tools prettierd ripgrep lua-language-server vscode-extensions.ms-vscode.cpptools vscode-langservers-extracted pyright];
